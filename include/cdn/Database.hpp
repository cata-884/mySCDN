...
public:
    ...
    void RegisterFile(const std::string& node_identificator, const std::string& nume_fisier);
    
    //lista de {fisier, nod}
    std::vector<std::pair<std::string, std::string>> GetCatalog();

    void LogAccess(const std::string& id_nod, const std::string& file, const std::string& ipAddress);
