...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "   -> Max Conexiuni Logice: " << config.maxConexiuni << std::endl;
        std::cout << "   -> Thread Pool Size: " << poolSize << std::endl;

        while(1) {
            TcpSocket clientSocket = tcpServer.Accept();
            //folosim shared pointer deoarece std::function cere ca tot ce pui in el sa fie copiabil, lucru pe care TcpSocket nu il are(putem face doar move)
            std::shared_ptr<TcpSocket> socketPtr = std::make_shared<TcpSocket>(std::move(clientSocket));
