...
TcpSocket::TcpSocket(int fd, std::string _ip, std::uint16_t _port) : sockFD(fd), ip(std::move(_ip)), port(_port) {}
TcpSocket::~TcpSocket() { Close(); }

TcpSocket::TcpSocket(TcpSocket&& other) noexcept : sockFD(other.sockFD), ip(std::move(other.ip)), port(other.port) {
    other.sockFD = -1;
    other.port = 0;
}
...
void TcpSocket::Bind(std::uint16_t _port, const std::string& ipAdress){
    ...
    throwIF(bind_status < 0, ("Eroare la bind folosind IP-ul " +  (ipAdress.empty() ? "ANY" : ipAdress) + " si portul " + std::to_string(port)).c_str());
}
...
TcpSocket TcpSocket::Accept() const{
    ...
    while (clientFD < 0 && (errno == EINTR || errno == EAGAIN)) {
        clientFD = ::accept(sockFD, reinterpret_cast<sockaddr*> (&clientAddr), &len);
    }
...
void TcpSocket::Connect(std::uint16_t _port, const std::string& ipAdress) {
    ...
    throwIF(::inet_pton(AF_INET, ipAdress.c_str(), &addr.sin_addr) <= 0, ("Adresa IP invalida la connect: " + ipAdress).c_str());
...
void TcpSocket::SendAll(const void* mesaj, std::size_t len) const {
    ...
    //trimitem pe chunks
    while(total < len){
        std::size_t trimis = static_cast<std::size_t>(::send(sockFD, buff + total, len-total, 0));
...
std::string TcpSocket::recvLine(std::size_t len) const{
    ...
    while(linie.size() < len){
        ...
        if (primit < 0 && errno == EINTR) continue;
