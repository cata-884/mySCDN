...
#include <algorithm>
//"ceva.mp3" => Σ (int)c
//daca Serverul A și Serverul B calculează hash-ul pentru "video.mp4", ambele vor obtine exact acelasi numar. Acest lucru este critic pentru ca toate nodurile să fie de acord asupra locației fișierului
...
hashRing::hashRing(const NodeConfig& config) : serviceNodes(std::move(config.peersVector)) {
...
//folosit pentru identificarea carui nod este responsabil, DACA nodul este online. daca e offline, se recalculeaza distributia
...
PeerDescriptor hashRing::NextAfter(const std::string& idNod) const{
    std::lock_guard<std::mutex> lock(threadMutex);
    if(serviceNodes.size() <= 1) return PeerDescriptor{};
