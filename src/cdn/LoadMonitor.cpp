...
//replicam std::mutex si distrugerea automata fara a apela explicit un destructor cand avem return sau throw
loadMonitor::ticket loadMonitor::tryAquire() {
    std::lock_guard<std::mutex> lock(m_mutex);

    if (clientiConectati < conexiuniMaxime) {
        clientiConectati++;
        //shared_from_this() ne permite sa avem o instanta de shared pointer atunci cand tot ce avem e this. vrem mereu sa tinem minte originalul
        try{
